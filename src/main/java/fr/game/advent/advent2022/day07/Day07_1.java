...
public class Day07_1 {
	...
	public static void main(String[] args) throws IOException {
		try {
			...
			LISTE_DOSSIERS.stream().map(Dossier -> Dossier.getTaille()).filter(Dossier -> Dossier <= 100000).forEach(System.out::println);
			int tailleCumulee = 0;
			tailleCumulee = (LISTE_DOSSIERS.stream().mapToInt(Dossier -> Dossier.getTaille()).filter(Dossier -> Dossier <= 100000)).sum();

//			tailleCumulee = LISTE_DOSSIERS.stream().mapToInt(Dossier -> Dossier.getTaille()).limit(100000).sum();
			System.out.println(tailleCumulee);
//			LISTE_DOSSIERS.stream().forEach(System.out::println);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private static void lireLigne() {
		...
		if (LIGNE.substring(0, 4).equals("$ cd")) {
			if (LIGNE.substring(5, 6).equals("/")) {
				...
				DOSSIER_ACTUEL = LISTE_DOSSIERS.stream().filter(Dossier -> Dossier.getNom().contains(NomDossierDestination)).filter(Dossier -> Dossier.getDossierParent() == DOSSIER_ACTUEL).findAny().get();
			}
		} else {
